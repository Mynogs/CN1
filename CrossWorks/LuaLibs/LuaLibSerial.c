...
LIB_FUNCTION(LuaSend, "count|E.C,send)")
{
  ...
  int result = HAL_SerialSend(channel, send, size);
  if (result < 0) 
    return LuaUtils_Error(L, HAL_ErrorToString(result));
  return result;
}
...
LIB_FUNCTION(LuaRecv, "recv|E.C")
{
  ...
  uint32_t channel = luaL_checkinteger(L, 1);
  int result = HAL_SerialRecv(channel, 0, 0);
  if (result < 0) 
    return LuaUtils_Error(L, HAL_ErrorToString(result));
  if (!result) {
    lua_pushstring(L, "");
  ...
  result = HAL_SerialRecv(channel, recv, result);
  if (result < 0) 
    return LuaUtils_Error(L, HAL_ErrorToString(result));
...
LIB_END
/*---------------------------------------------------------------------------*/
void LuaLibSerial_Register(lua_State *L)
{
  LuaUtils_Register(L, "sys", "serial", Lib);
}
/*---------------------------------------------------------------------------*/
